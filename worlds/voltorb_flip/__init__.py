...
class VoltorbFlipWorld(World):
    ...
    progression_list: ClassVar[list[tuple[str, int]]] = (
        []
    )  # Only used in set_rules, so reusable for other slots
    ...
    def __init__(self, multiworld: MultiWorld, player: int):
        ...
        self.max_level = 0
        self.last_coin = 0
        self.luck_classification = (
            ItemClassification.progression_deprioritized_skip_balancing
        )

    def create_item(self, name: str) -> "Item":
        return VoltorbFlipItem(
            name, self.luck_classification, self.item_name_to_id[name], self.player
        )
    ...
    def create_regions(self) -> None:
        ...
        for i in range(1, max_level + 1):
            if i == 1 or i / max_level <= 0.4:
                self.regions["Earlier levels"].locations.append(
                    locations.VoltorbFlipLocation(
                        self.player, f"Win level {i}", i, self.regions["Earlier levels"]
                    )
                )
            elif i == max_level:
                self.regions["Last level"].locations.append(
                    locations.VoltorbFlipLocation(
                        self.player, f"Win level {i}", i, self.regions["Last level"]
                    )
                )
            else:
                self.regions["Later levels"].locations.append(
                    locations.VoltorbFlipLocation(
        ...
        def add_coin_location(region_name: str, coin_amount: int):
            self.regions[region_name].locations.append(
                locations.VoltorbFlipLocation(
                    self.player,
                    f"Collect {coin_amount} coins",
                    coin_amount,
        ...
        for i in range(coin_steps, last_coin + 1, coin_steps):
            if coin_regions != -1:
                ...
                add_coin_location(
                    create_coin_region_name(region_index, coin_regions), i
                )
    ...
    def set_rules(self) -> None:
        ...
        if (
            ...
            and not settings.get_settings()["voltorb_flip_settings"][
                "allow_experimental_logic"
            ]
        ):
            import logging

            logging.warning(
                "Experimental logic was disabled in host settings, so it will be reverted to non-experimental for player "
                + self.player_name
                + "."
            )
        ...
        if self.options.artificial_logic == "experimental":
            ...
            self.regions["Earlier levels"].connect(
                ...
                "Later levels",
                lambda state: state.has(items[0][0], items[0][1]),
            )
            self.regions["Later levels"].connect(
                self.regions["Last level"],
                "Last level",
                lambda state: state.has(items[1][0], items[1][1]),
            )
            ...
            if custom_coin_regions == -1:
                self.regions["Early coins"].connect(
                    ...
                    "Mid coins",
                    lambda state: state.has(items[2][0], items[2][1]),
                )
                self.regions["Mid coins"].connect(
                    self.regions["Late coins"],
                    "Late coins",
                    lambda state: state.has(items[3][0], items[3][1]),
                )
            ...
            self.regions["Later levels"].connect(
                self.regions["Last level"], "Last level"
            )
        ...
        if self.options.goal == "levels":
            self.multiworld.completion_condition[self.player] = (
                lambda state: state.can_reach_location(
                    f"Win level {self.max_level}", self.player
                )
            )
        else:
            self.multiworld.completion_condition[self.player] = (
                lambda state: state.can_reach_location(
                    f"Collect {self.last_coin} coins", self.player
                )
            )
        self.regions.clear()
