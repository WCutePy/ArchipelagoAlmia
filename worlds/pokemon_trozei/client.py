...
class PokemonTrozeiCient(BizHawkClient):
    ...
    async def validate_rom(self, ctx: "BizHawkClientContext") -> bool:
        header = await bizhawk.read(
            ctx.bizhawk_ctx, ((0x00000000, 0x0B, self.rom_read_only_domain),)
        )
        ...
        if cmd == "ReceivedItems":
            if len(self.local_unlocked_stages) == 0:
                for item in ctx.items_received:
                    ...
                    if item_id == 110:
                        asyncio.create_task(self.turn_on_hardmode(ctx))
        elif cmd == "Connected":
            ...
    ...
    async def game_watcher(self, ctx: "BizHawkClientContext") -> None:
        ...
        try:
            ...
            if (
                ctx.slot_data.get("death_link", Toggle.option_false)
                == Toggle.option_true
            ):
            ...
            endless_score = await bizhawk.read(
                ctx.bizhawk_ctx, ((0x021222D5, 5, self.ram_read_write_domain),)
            )
            buf = bytearray(endless_score[0])
            if not (
                (endless_score[0][3] >= 0xE0) and (endless_score[0][4] & 0x01) == 1
            ):
                buf[0] &= 0b111
                buf[1] = 0x00
                buf[2] = 0x00
                ...
                await bizhawk.write(
                    ...
                    (
                        ...
                        (0x02121E65, (0).to_bytes(), self.ram_read_write_domain),
                    ),
                )
                num_receieved_items = 0
            else:
                num_receieved_items = int.from_bytes(
                    endless_score[0][1:3], "little", signed=False
                )
            ...
            for i, stage in enumerate(data.stages):
                ...
                if is_unlocked and i not in self.local_unlocked_stages:
                    await self.write_stage_location(
                        ctx, locations[byte : byte + 2], byte, bit1, 0b00
                    )
                    continue
                elif (
                    not is_unlocked
                    and i in self.local_unlocked_stages
                    and i not in handle_manually
                ):
                    to_write = 0b01
                    if stage_id_to_location_id(i) in ctx.checked_locations:
                        to_write = 0b11
                    await self.write_stage_location(
                        ctx, locations[byte : byte + 2], byte, bit1, to_write
                    )
            ...
            if (
                35 in self.local_unlocked_stages
                and sum(
                    stage_id_to_location_id(i) in ctx.checked_locations
                    for i in range(30, 35)
                )
                >= ctx.slot_data["required_bosses"]
            ):
    ...
    async def handle_death_link(
        self, ctx: "BizHawkClientContext", death_link_only: bool = False
    ) -> None:
        ...
        if not currently_dead:
            ...
            await ctx.send_death(
                f"{ctx.player_names[ctx.slot]} could not link fast enough!"
            )
    ...
    async def write_stage_location(
        self,
    ...
    async def handle_received_items(
        ...
        while num_received_items < len(ctx.items_received):
            ...
            if item_id < 100:
                ...
                write = 0b01
                if (
                    stage_id * len(data.stages[0].location_names) + 1
                ...
                if stage_id != 35:
                    ...
                    await self.write_stage_location(
                        ctx, read_result[0], byte, bit1, write
                    )
                ...
                new_val = (
                    int.from_bytes(read_result[0], "little", signed=False)
                    + [1, 3, 5][item_id - 100]
                )
                ...
                await bizhawk.write(
                    ctx.bizhawk_ctx,
                    [(0x02121E65, new_bytes, self.ram_read_write_domain)],
                )
            elif item_id == 110:
                await self.turn_on_hardmode(ctx)
            else:
                continue
    ...
    async def turn_on_hardmode(
        self,
        ctx: "BizHawkClientContext",
    ):
        print("Turning on hardmode!!")
        read_result = await bizhawk.read(
            ctx.bizhawk_ctx,
            [
                (0x02122BD2, 1, self.ram_read_write_domain),
            ],
        )
        value = read_result[0][0]
        new_value = value | 0x10

        succes = await bizhawk.guarded_write(
            ctx.bizhawk_ctx,
            [
                (
                    0x02122BD2,
                    new_value.to_bytes(1, "little", signed=False),
                    self.ram_read_write_domain,
                )
            ],
            [
                (0x02122BD2, read_result[0], self.ram_read_write_domain),
            ],
        )
